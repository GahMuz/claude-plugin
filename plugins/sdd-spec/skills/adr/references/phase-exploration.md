...
Lire `framing.md`. Garder en tête les contraintes et non-objectifs — ils élimineront certaines options d'emblée.
...
Agent({
  ...
  subagent_type: "sdd-spec:spec-deep-dive",
  model: "opus",
...
Si complet → "Lancez `/adr approve` pour passer à la phase de discussion."

### Step 8: Rule candidates

Si pendant l'exploration une convention architecturale mérite d'être généralisée (ex: "toujours évaluer le licensing avant de choisir un outil open source", "préférer les solutions AWS-native si déjà sur AWS"), ajouter dans `rule-candidates.md` :
