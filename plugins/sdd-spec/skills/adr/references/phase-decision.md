...
Lire :
- `framing.md` — problème, contraintes, non-objectifs
- `options.md` — toutes les options avec trade-offs
- `log.md` — arguments clés de la discussion, option choisie

### Step 2: Rédiger adr.md

Écrire `.sdd/decisions/YYYY/MM/<adr-id>/adr.md` :
...
"Lancez `/adr approve` pour finaliser cet ADR et passer à la transition."

### Step 5: Rule candidates

Si la décision elle-même constitue une règle généralisable (ex: "utiliser Spring @Transactional plutôt que Jakarta dans ce projet"), ajouter dans `rule-candidates.md` :

```
## [decision] <règle en une ligne>
- **Domaine** : <architecture|sécurité|infra|api|service|transversal>
- **Contexte** : décision de l'ADR-xxx — <résumé>
...
```
Décision formalisée : <option choisie>. ADR-xxx rédigé.
```
