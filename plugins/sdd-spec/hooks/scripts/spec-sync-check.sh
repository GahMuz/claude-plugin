...
# Reads .sdd/local/active.json — the single source of truth for the active item (spec or ADR).
...
EOF
  ...
  cat <<EOF
{"systemMessage": "Spec actif : ${ITEM_ID} (phase : ${PHASE}). Si le message utilisateur contient un retour affectant les exigences, la conception ou le plan, mettre à jour les documents concernés en place et loguer dans state.json changelog."}
EOF
fi
