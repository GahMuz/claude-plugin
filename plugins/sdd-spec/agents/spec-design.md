...
1. Lire `<specPath>/requirement.md` — extraire les REQs listés dans `newREQIds` avec leurs critères d'acceptation
2. Lire `<specPath>/design.md` — identifier le dernier DES-xxx ID, charger les patterns architecturaux existants
3. Glob `**/sdd-rules/SKILL.md` → exécuter le protocole de chargement — les règles orientent les choix techniques
4. Si nécessaire, dispatcher `spec-deep-dive` pour explorer le code existant autour des zones impactées :
   ```
   Agent({ subagent_type: "sdd-spec:spec-deep-dive", model: "opus", prompt: "<question ciblée>" })
   ```
...
Mettre à jour `<specPath>/state.json` changelog :
```json
{ "date": "<ISO-8601>", "type": "design-update", "description": "X DES ajoutés depuis revue", "addedItems": ["DES-xxx", "DES-yyy"] }
```
