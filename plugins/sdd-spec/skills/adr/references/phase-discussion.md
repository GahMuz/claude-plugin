...
- Proposer une recherche ciblée via `spec-deep-dive`
...
Si oui → "Lancez `/adr approve` pour formaliser la décision."
Si non → continuer la discussion.

### Step 7: Rule candidates

Si pendant la discussion un pattern ou une convention émerge qui s'applique au-delà de cet ADR (ex: "quand on choisit entre X et Y, toujours prioriser la solution la moins couplée à un vendor"), ajouter dans `rule-candidates.md` :
